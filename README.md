# Zepkit: Real-time Order Tracking with Android Baklava

Zepkit is a concept food-delivery tracking application designed to showcase the power of **Android Baklava’s** new `ProgressStyle` notifications. It mimics the high-fidelity tracking experience seen in apps like Zepkit, using smooth, segmented progress bars and custom tracker icons.
...
* **`LiveNotificationManager`**: The core logic for styling the notification, handling Zepkit-themed colors, and configuring the new Baklava segments.
...
```bash
git clone https://github.com/yourusername/Zepkit.git
...
The app  signature color palette:

* **GREEN**: `#5B2DFF` (Primary Branding)
* **GRAY**: `#FAB9B9` (Inactive Segments)
* **DONE**: `#27AE60` (Completion State)
